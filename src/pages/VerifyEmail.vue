<template>
  <div class="max-w-lg w-full mx-auto p-2">
    ...
    <p v-if="!user?.emailVerified" class="text-lg text-red-600 xl:text-xl mt-4">
      ...
      <span class="text-primary">{{ user?.email }}</span> Please check your
      email and click link to verified your account
    </p>
    <button
    ...
    <p class="text-lg mt-6 xl:text-xl text-right">
      ...
      <span v-if="isPending" class="text-sky-600 cursor-wait">
        Loading...
      </span>
      <span
        v-else
